...
test('shipped LLM-first client executes canonical payloads, errors, blocked drafts, review, and export', async () => {
  ...
  await elements.get('go').onclick(); assert.equal(elements.get('error').textContent, 'We could not reach the resume service. Check your connection and retry.'); assert.equal(elements.get('readiness').textContent, 'The resume service is temporarily unavailable. Retry.'); assert.match(elements.get('diagnostics').textContent, /invalid_structured_response.*openai-compatible.*safe-model.*Extracted: 0.*Validation reasons: invalid_structured_response/);
