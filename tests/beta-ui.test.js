...
test('shipped LLM-first client executes canonical payloads, errors, blocked drafts, review, and export', async () => {
  ...
  await elements.get('go').onclick(); assert.equal(elements.get('error').textContent, 'We couldn’t identify enough usable information from this resume. Review the file and try again.'); assert.doesNotMatch(elements.get('error').textContent, /connection/i); assert.match(elements.get('diagnostics').textContent, /invalid_structured_response.*openai-compatible.*safe-model.*Extracted: 0.*Validation reasons: invalid_structured_response/);
