...
test('unknown SQL need with explicit confirmed profile skill is accepted, sufficient, and skips lower sources', () => withStore((store) => {
  ...
  assert.deepEqual(run.source_searches.map((search) => search.result_status), ['completed_no_candidates', 'completed_with_candidates', 'skipped_sufficient', 'skipped_sufficient', 'skipped_sufficient']);
}));
...
test('no evidence completes local sources and creates no question', () => withStore((store) => {
  ...
  assert.equal(run.source_searches.length, 5);
