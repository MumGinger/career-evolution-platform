...
function plan() {
  ...
  return {
    ...
    candidate_knowledge_snapshot: [projectFact],
    ...
    section_plans: [{ section: 'Projects', recommended_order: 1, candidate_fact_ids: [projectFact.id] }],
  };
}
...
test('tailored project heading keeps source entry identity through composition, Career Review, and presentation', () => {
  const provider = {
  ...
  const giftBody = projects.statements.find((statement) =>
    statement.text === 'Built a recommendation workflow with React and API integrations.');
  ...
  assert.equal(giftBody.parent_source_statement_id, tailoredHeading.source_statement_id);
  ...
  assert.deepEqual(projectModel.entries[0].body.map((statement) => statement.text), [
    'Built a recommendation workflow with React and API integrations.',
  ]);
