...
test('draft statement citing an unsupported fact is blocked by deterministic validation', () => {
  ...
  const artifactRun = { id: 'artifact-run', resume_tailoring_plan_run_id: 'plan-run', resume_artifacts: [{ content: { sections: [{ section: 'Skills', position: 1, statements: [{ statement_id: 'draft:Skills:1', template: 'skill_name', text: 'SQL', resume_content_selection_ids: [selection.id], provenance: { candidate_fact_id: 'fact-sql', candidate_fact_ids: ['fact-sql', 'invented-fact'], job_requirement_ids: ['requirement-sql', 'invented-requirement'], inherited_provenance_references: ['decision-sql'] } }] }] }, metadata: { traceability: { resume_artifact_run_id: 'artifact-run', resume_tailoring_plan_run_id: 'plan-run' }, omissions: [], blocked_claims: [], requirement_coverage: [], draft_provider: { provider: 'openai-compatible', model: 'test' } } }] };
  ...
  assert.ok(result.findings.some((item) => item.rule === 'draft-fact-citation-set'));
  assert.ok(result.findings.some((item) => item.rule === 'draft-requirement-citation-set'));
});
