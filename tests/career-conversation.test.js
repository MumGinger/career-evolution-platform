...
test('fixture CLI adds the optional answer to JSON and HTML output', async () => {
  ...
  try { await reviewCli(['--db', db, '--candidate-profile-id', knowledge.profile.id, '--job-profile-id', job.id, '--review-fixture', fixture, '--non-interactive', '--output-dir', output], { stdin: null, stdout: { write() {} } }); const saved = JSON.parse(fs.readFileSync(path.join(output, 'career-conversation.json'), 'utf8')); const snapshot = JSON.parse(fs.readFileSync(path.join(output, 'career-understanding-snapshot.json'), 'utf8')); const report = fs.readFileSync(path.join(output, 'evidence-review-report.html'), 'utf8'); assert.equal(saved.answer, 'AI / ML'); assert.equal(snapshot.current_direction.label, 'AI / ML'); assert.match(report, /Career Conversation/); assert.match(report, /Current Career Snapshot/); assert.match(report, /You selected AI \/ ML/); } finally { fs.rmSync(dir, { recursive: true, force: true }); }
});
test('interactive review closes its reader before asking the career question', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'career-conversation-interactive-')); const db = path.join(dir, 'test.db'); const store = new Store(db); const knowledge = store.createResumeProfile({ sourcePath: 'synthetic.pdf', basic: { name: 'Synthetic Candidate' }, facts: [{ entity_type: 'skill', value: { name: 'SQL' }, confirmation_status: 'needs_confirmation' }] }); const job = store.createJobRequirementProfile({ company: 'Acme', roleTitle: 'Analyst', jobDescription: 'Required Qualifications:\nSQL required.' }); store.close(); const output = path.join(dir, 'output'); let terminal = ''; let prompts = 0; const stdin = new Readable({ read() {} }); const io = { stdin, stdout: new Writable({ write(chunk, encoding, callback) { terminal += chunk.toString(); if (chunk.toString().includes('Choice: ')) { prompts += 1; setImmediate(() => stdin.push(prompts === 1 ? '2\n' : '1\n')); } callback(); } }) };
  try { const result = await reviewCli(['--db', db, '--candidate-profile-id', knowledge.profile.id, '--job-profile-id', job.id, '--output-dir', output], io); assert.equal(prompts, 2); assert.equal(result.careerConversation.answer, 'Data Analytics'); assert.match(terminal, /Resume generated successfully/); } finally { stdin.push(null); fs.rmSync(dir, { recursive: true, force: true }); }
});
