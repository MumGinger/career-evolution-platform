...
class Store {
  ...
  async createResumeAstRun({ profileId, artifactId, provider, promptVersion = resumeAst.PROMPT_VERSION }) {
    const artifact = this.getSourceResumeArtifact(artifactId); if (artifact.profile_id !== profileId) throw new Error('Source resume artifact does not belong to candidate profile'); const version = artifact.versions.at(-1); const result = await provider.extractResumeAst({ text: version.parsed_text, schema: resumeAst.RESUME_AST_JSON_SCHEMA, promptVersion });
