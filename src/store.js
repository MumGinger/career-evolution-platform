...
class Store {
  ...
  createResumeAstRun({ profileId, artifactId, provider, promptVersion = resumeAst.PROMPT_VERSION }) {
    const artifact = this.getSourceResumeArtifact(artifactId); if (artifact.profile_id !== profileId) throw new Error('Source resume artifact does not belong to candidate profile'); const version = artifact.versions.at(-1); const result = provider.extractResumeAst({ text: version.parsed_text, schema: { version: resumeAst.SCHEMA_VERSION }, promptVersion });
    if (result && typeof result.then === 'function') throw new Error('Async providers require the dedicated async extraction entrypoint; use mock in the local Demo.');
    const checked = resumeAst.validateAst({ ast: result.ast, text: version.parsed_text }); const run = { id: this.id(), profile_id: profileId, artifact_id: artifactId, artifact_version_id: version.id, extractor_provider: result.provider, extractor_model: result.model, extractor_version: result.version, prompt_version: promptVersion, schema_version: resumeAst.SCHEMA_VERSION, raw_response_hash: resumeAst.hash(result.rawResponse), normalized_ast: JSON.stringify(checked.normalizedAst), validation_status: checked.validation_status, validation_findings: JSON.stringify(checked.findings), created_at: this.now() };
  ...
  getResumeAstEvidenceSnapshot(profileId) {
    return this.db.prepare("SELECT * FROM resume_ast_runs WHERE profile_id = ? AND validation_status = 'passed' ORDER BY created_at, id").all(profileId).flatMap((run) => {
      ...
      const add = (leaf, entity_type, section) => { if (leaf) rows.push({ id: `${run.id}:${rows.length}`, entity_type, value: { name: leaf.value, text: leaf.value }, source: 'resume_ast', confirmation_status: leaf.provenance.extraction_state === 'explicit' ? 'confirmed' : 'needs_confirmation', provenance: { resume_ast_run_id: run.id, artifact_id: run.artifact_id, artifact_version_id: run.artifact_version_id, section, ...leaf.provenance } }); };
      ...
      (ast.experiences || []).forEach((item) => { add(item.organization, 'organization', 'experiences'); add(item.role, 'role', 'experiences'); (item.bullets || []).forEach((value) => add(value, 'bullet', 'experiences')); }); return rows;
    });
  }
  ...
  createEvidenceDiscoveryRun({ informationNeedRunId, informationNeedId }) {
    ...
    for (const need of selected) {
      ...
      for (const [index, sourceType] of discovery.SOURCE_ORDER.entries()) {
        const sourceFacts = (sourceType === 'resume_ast' ? astSnapshot : sourceType === 'resume_semantic' ? semanticSnapshot : sourceType === 'resume_semantic_graph' ? graphSnapshot : snapshot).filter((fact) => discovery.sourceFor(fact) === sourceType);
