...
class Store {
  ...
  getInformationNeedRun(id) {
    ...
    const graphEvidence = this.getResumeSemanticGraphEvidenceSnapshot(run.candidate_profile_id);
    const available_working_evidence_summary = graphEvidence.map((item) => ({ source: item.source, node_type: item.entity_type, label: item.value.name, decision_state: item.provenance.decision_state, graph_run_id: item.provenance.graph_run_id, evidence_span_ids: item.provenance.evidence_span_ids }));
  ...
  createEvidenceDiscoveryRun({ informationNeedRunId, informationNeedId }) {
    ...
    const semanticSnapshot = this.getResumeSemanticEvidenceSnapshot(informationNeedRun.candidate_profile_id); const graphSnapshot = this.getResumeSemanticGraphEvidenceSnapshot(informationNeedRun.candidate_profile_id);
    const run = { id: this.id(), information_need_run_id: informationNeedRunId, discovery_policy_version: discovery.POLICY_VERSION, source_snapshot: JSON.stringify({ candidate_evidence: snapshot, resume_semantic: semanticSnapshot, resume_semantic_graph: graphSnapshot }), selected_information_need_ids: JSON.stringify(selected.map((need) => need.id)), created_at: this.now() };
    ...
    for (const need of selected) {
      ...
      for (const [index, sourceType] of discovery.SOURCE_ORDER.entries()) {
        const sourceFacts = (sourceType === 'resume_semantic' ? semanticSnapshot : sourceType === 'resume_semantic_graph' ? graphSnapshot : snapshot).filter((fact) => discovery.sourceFor(fact) === sourceType);
