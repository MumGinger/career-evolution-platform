...
function deterministicDraft({ selections, facts }) {
  ...
  for (const selection of selections.filter((item) => item.selection_state === 'include')) {
    const fact = facts.get(selection.candidate_fact_id); const text = display(fact); if (!text) continue;
...
class OpenAiCompatibleResumeDraftProvider {
  ...
  async draft({ facts, selections, requirements, presentationStrategy }) {
    const committed = selections.filter((item) => item.selection_state === 'include').map((selection) => ({ candidate_fact_id: selection.candidate_fact_id, recommended_section: selection.recommended_section, permitted_claim_scope: selection.permitted_claim_scope, mapped_job_requirement_ids: selection.mapped_requirement_ids, value: display(facts.get(selection.candidate_fact_id)) }));
