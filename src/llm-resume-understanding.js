...
class OpenAiCompatibleResumeUnderstandingProvider {
  ...
  async understand({ text }) { const response = await fetch(`${this.baseUrl.replace(/\/$/, '')}/chat/completions`, { method: 'POST', headers: { Authorization: `Bearer ${this.apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: this.model, response_format: { type: 'json_object' }, messages: [{ role: 'system', content: 'Return JSON {blocks:[]}. Each block has id,type,title,label,exact_source_text,source_location,parent_id,normalized_meaning,confidence,state,provenance,limitations. Use only exact resume text; preserve experience/project to responsibility/achievement relationships and mark uncertainty explicitly.' }, { role: 'user', content: text }] }) }); if (!response.ok) throw new Error(`LLM Resume Understanding provider failed: ${response.status}`); const raw = (await response.json()).choices?.[0]?.message?.content; if (!raw) throw new Error('LLM Resume Understanding provider returned no content.'); return { provider: this.name, model: this.model, understanding: JSON.parse(raw) }; }
}
...
module.exports = { BLOCK_TYPES, MockResumeUnderstandingProvider, OpenAiCompatibleResumeUnderstandingProvider, providerFromConfig, mockUnderstand, validateUnderstanding, grouped, approvedBlocks, draft, validateDraft };
