const POLICY_VERSION = 'resume-tailoring-policy/1.0.0';
...
function claimScope(fact, supportingProject) {
  if (fact.entity_type === 'skill') return supportingProject ? ['skill_name', 'used_in_linked_project'] : ['skill_name'];
  if (fact.entity_type === 'achievement') return ['accepted_achievement_detail'];
  if (fact.entity_type === 'project') return ['project_name', 'bounded_project_responsibilities'];
  if (fact.entity_type === 'responsibility') return ['bounded_responsibility'];
