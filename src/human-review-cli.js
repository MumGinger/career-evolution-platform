...
function html(run, exported) { return `<!doctype html><meta charset="utf-8"><title>Career Review</title><style>body{font:16px system-ui;max-width:900px;margin:2rem auto;line-height:1.5}section{border:1px solid #ddd;padding:1rem;margin:1rem 0}pre{white-space:pre-wrap}</style><h1>Career Review — Resume</h1><p><strong>Completion:</strong> ${run.review_complete ? 'Complete — export allowed' : 'Incomplete — export blocked'}</p><p>The AI prepares. The human decides. Every presentation decision is transparent and reviewable before export.</p>${run.section_reviews.map((review) => `<section><h2>${escapeHtml(review.section)}</h2><p><strong>Action:</strong> ${escapeHtml(review.action)} at ${escapeHtml(review.reviewed_at)}</p><h3>AI version</h3><pre>${escapeHtml(JSON.stringify(review.ai_version, null, 2))}</pre><h3>Supporting evidence / provenance</h3><pre>${escapeHtml(JSON.stringify(review.supporting_evidence, null, 2))}</pre><h3>Why this presentation</h3><ul>${review.presentation_rationale.map((reason) => `<li>${escapeHtml(reason)}</li>`).join('')}</ul><h3>Final approved version</h3><pre>${escapeHtml(JSON.stringify(review.final_version, null, 2))}</pre></section>`).join('')}<h2>Approved Resume Export</h2><pre>${escapeHtml(exported.markdown)}</pre>`; }
...
async function main(argv = process.argv.slice(2), io = process) {
  ...
  try {
    ...
    io.stdout.write(`Career Review complete: ${run.section_reviews.length}/${humanReview.REQUIRED_SECTIONS.length} required sections explicitly reviewed.\nApproved Resume Export: allowed\nOutputs: ${outputDir}\n`);
