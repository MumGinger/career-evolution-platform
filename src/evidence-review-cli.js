...
function careerAdapter(input, output) {
  const rl = readline.createInterface({ input, output });
  return { ask: async () => {
    output.write(`\nResume generated successfully.\n\nOne optional question\n(helps me better understand your career over time)\n\n${conversation.QUESTION}\n${conversation.OPTIONS.map((option, index) => `[${index + 1}] ${option}`).join('  ')}  [S] Skip\n`);
    const choice = (await rl.question('Choice: ')).trim().toLowerCase();
    if (choice === 's' || choice === 'skip') return { skipped: true };
    const answer = conversation.OPTIONS[Number(choice) - 1];
    if (!answer) throw new Error('Choose an option or S to skip');
    return { answer };
  }, close: () => rl.close() };
}
...
function html(result) {
  ...
  const observation = result.careerConversation;
  const career = observation ? `<h2>Career Conversation</h2><p><strong>Question:</strong> ${escapeHtml(observation.question)}<br><strong>Answer:</strong> ${observation.skipped ? 'Skipped' : escapeHtml(observation.answer)}<br><strong>Skipped?</strong> ${observation.skipped ? 'Yes' : 'No'}</p>` : '';
  const snapshot = result.careerSnapshot ? `<h2>Current Career Snapshot</h2><pre>${escapeHtml(understanding.readable(result.careerSnapshot))}</pre>` : '';
  return `<!doctype html><meta charset="utf-8"><title>Evidence Review</title><h1>Evidence Review</h1><h2>Review queue</h2><p>Needs review: ${result.kpi.needs_review_count}; auto accepted: ${result.kpi.auto_accepted_evidence_count}</p><ul>${decisions}</ul><h2>Outcome</h2><p>Coverage: ${result.kpi.committed_coverage_before}% to ${result.kpi.committed_coverage_after}%<br>Candidate Knowledge facts: ${result.kpi.candidate_knowledge_facts_before} to ${result.kpi.candidate_knowledge_facts_after}<br>Validation: ${escapeHtml(result.validation.status)}</p>${career}${snapshot}<h2>Resume preview</h2><pre>${escapeHtml(result.artifact.markdown)}</pre><h2>Validation findings</h2><pre>${escapeHtml(JSON.stringify(result.validation.findings, null, 2))}</pre>`;
}
async function main(argv = process.argv.slice(2), io = process) {
  const input = options(argv); const store = new Store(input.db || 'career-evolution.db'); let prompt; let careerPrompt;
  try {
    ...
    io.stdout.write(`${JSON.stringify(result.kpi, null, 2)}\nCareer Snapshot: direction ${result.careerSnapshot.current_direction.state}; items ${result.careerSnapshot.understanding_items.length}; unknowns ${result.careerSnapshot.unknowns.length}; feedback ${result.careerSnapshot.feedback_state}\nOutputs: ${outputDir}\n`); return result;
  } finally { if (prompt) prompt.close(); if (careerPrompt) careerPrompt.close(); store.close(); }
}
if (require.main === module) main().catch((error) => { console.error(`Error: ${error.message}`); process.exitCode = 1; });
module.exports = { main, adapter, careerAdapter, html };
