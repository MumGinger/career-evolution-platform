...
Validation: focused tests 19/19 passing; full `npm.cmd test` 156/156 passing; `node --check` for all changed JavaScript files; `git diff --check` passing. Ya-Ching/Zurich regression remains green: accepted project and responsibility evidence reaches 003.6 and renders non-empty Experience and Projects sections.
